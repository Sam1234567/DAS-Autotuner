...
# 4 150 0.01 0.5 50000 5.0 2.0
# 4 300 0.005 0.5 50000 5.0 2.0
if(len(sys.argv) > 2):
	N = int(sys.argv[2])
...
w_init_noisyopt = w_init
dt_noisyopt = 0.1

if(not fixed_w):
	indir_noisyopt = indir_noisyopt + "/D_%i_w_init_%f_dt_%f_nsamp_max_%i" % (D, w_init_noisyopt, dt_noisyopt, nsamp_max)
else:
	indir_noisyopt = indir_noisyopt + "/D_%i_w_init_%f_dt_%f_nsamp_max_%i_fw" % (D, w_init_noisyopt, dt_noisyopt, nsamp_max)

indir_noisyopt = indir_noisyopt + "_" + str(method)

indir_noisyopt = indir_noisyopt + "/N_%i_T_%f_beta_%f" % (N,T, temp_beta)


indir_list = []

indir_list.append("Ising_test_ellipsoid" + indir)
indir_list.append("Ising_test" + indir)

indir_list.append("Ising_test_gasnikov" + indir)
indir_list.append("Ising_test_noisyopt" + indir_noisyopt)
indir_list.append("Ising_test_bohb" + indir)
